...
with st.sidebar:
    ...
    st.caption("Cosmetic only — upscales & sharpens the display. It cannot add detail the model didn't generate; for real quality gains, retrain the Generator longer / at a higher resolution.")
    enhance_output = st.checkbox("Upscale & sharpen output", value=True)
    upscale_factor = st.select_slider("Upscale factor", options=[1, 2, 3, 4], value=4, disabled=not enhance_output)
    sharpen_amount = st.slider("Sharpen strength", 0, 200, 120, step=10, disabled=not enhance_output)
...
def enhance_images(images, upscale_factor=1, sharpen_amount=0):
    """Cosmetic post-processing: Lanczos upscale + unsharp mask + mild contrast boost.
    Does NOT add real detail — just makes the existing pixels look crisper/bigger."""
    if upscale_factor <= 1 and sharpen_amount <= 0:
        return images
    out = []
    for arr in images:
        im = Image.fromarray(arr)
        if upscale_factor > 1:
            new_size = (im.width * upscale_factor, im.height * upscale_factor)
            im = im.resize(new_size, resample=Image.LANCZOS)
        if sharpen_amount > 0:
            im = im.filter(ImageFilter.UnsharpMask(radius=2, percent=sharpen_amount, threshold=2))
...
if generate_btn:
    if model_file is None:
        st.error("⚠️ Please upload a Generator model file from the sidebar first.")
    else:
        ...
        if enhance_output:
            images = enhance_images(images, upscale_factor=upscale_factor, sharpen_amount=sharpen_amount)
        ...
        if do_interpolate:
            ...
            try:
                ...
                if enhance_output:
                    morph_images = enhance_images(morph_images, upscale_factor=upscale_factor, sharpen_amount=sharpen_amount)
    ...
    st.info("👈 Upload your Generator model, adjust the settings, then click **Generate Faces**.")
